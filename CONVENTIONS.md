...
The `base-form-demo-vault/` directory is a ready-to-use Obsidian vault. When adding new plugin features:

- Add representative sample notes to `base-form-demo-vault/` demonstrating the feature.
- Update `base-form-demo-vault/README.md` if new files require explanation.
- Do **not** commit `base-form-demo-vault/.obsidian/plugins/` build artifacts — those are regenerated by `scripts/build-to-demo-vault.ps1`.
